...
def parse_pages(spec: str, page_count: int) -> list[int]:
    """Parse a 1-based page spec like ``'4,7-9'`` into a sorted list of valid page numbers."""
    pages: set[int] = set()
...
def parse_bbox(spec: str | None) -> list[float] | None:
    """Parse a ``'x0,y0,x1,y1'`` crop spec (PDF points) into four floats, or None."""
    if spec is None:
        return None
...
def page_text(page: Any, mode: str) -> str:
    """Return a page's text, either as a plain dump or with per-block separation."""
    if mode == "blocks":
...
def extract_text(config: TextConfig) -> str:
    """Dump the text layer with a per-page stats header for modality classification.

    The header reports characters of extractable text, embedded raster count, and vector
    drawing count. A page with little text but many drawings is typeset math or a vector
    diagram — its text layer is unreliable, so it needs a visual pass instead.
    """
    fitz = import_fitz()
    doc = fitz.open(config.pdf)
    page_numbers = (
        parse_pages(config.pages, doc.page_count)
        if config.pages
    ...
    for page_number in page_numbers:
        ...
        chunks.append(
            ...
            f"images={n_images} drawings={n_drawings} ==="
        )
...
def render_pages(config: RenderConfig) -> list[Path]:
    """Rasterize the requested page(s) to PNG and return the written paths."""
    fitz = import_fitz()
    doc = fitz.open(config.pdf)
    page_numbers = parse_pages(config.pages, doc.page_count)
    bbox = parse_bbox(config.bbox)
    config.out_dir.mkdir(parents=True, exist_ok=True)
    matrix = fitz.Matrix(config.dpi / 72, config.dpi / 72)
    single = len(page_numbers) == 1
    written: list[Path] = []
    for page_number in page_numbers:
        page = doc[page_number - 1]
        ...
        name = (
            f"{config.prefix}.png" if single else f"{config.prefix}-p{page_number}.png"
        )
        out_path = config.out_dir / name
        pixmap.save(out_path)
        written.append(out_path)
        logger.info(f"Rendered page {page_number} → {out_path}")
    doc.close()
    return written


def extract_embedded(config: EmbeddedConfig) -> list[Path]:
    """Dump embedded raster images above the size threshold and return the written paths."""
    fitz = import_fitz()
    doc = fitz.open(config.pdf)
    config.out_dir.mkdir(parents=True, exist_ok=True)
    written: list[Path] = []
    ...
    for page in doc:
        for image in page.get_images(full=True):
            ...
            if base["width"] < config.min_size or base["height"] < config.min_size:
                continue
            out_path = (
                config.out_dir / f"{config.prefix}-{len(written) + 1:02d}.{base['ext']}"
            )
            out_path.write_bytes(base["image"])
            written.append(out_path)
            logger.info(f"Extracted embedded image xref={xref} → {out_path}")
    doc.close()
    if not written:
        logger.warning(
            "No embedded raster images passed the size filter — vector figures? try `render`."
        )
    return written
...
def text(pdf: Path, pages: str | None, mode: str) -> None:
    """Dump the text layer + per-page stats (chars/images/drawings). Read this first."""
...
def render(
    pdf: Path, pages: str, out_dir: Path, prefix: str, dpi: int, bbox: str | None
) -> None:
    """Rasterize page(s) — optionally cropped to --bbox — to PNG. Captures vector diagrams too."""
    config = RenderConfig(
        pdf=pdf, pages=pages, out_dir=out_dir, prefix=prefix, dpi=dpi, bbox=bbox
    )
...
@click.option(
    ...
    show_default=True,
    type=int,
    help="Skip images smaller than this (px)",
)
def embedded(pdf: Path, out_dir: Path, prefix: str, min_size: int) -> None:
    """Dump embedded raster images (clean photos). Vector diagrams won't appear — use `render`."""
    config = EmbeddedConfig(pdf=pdf, out_dir=out_dir, prefix=prefix, min_size=min_size)
