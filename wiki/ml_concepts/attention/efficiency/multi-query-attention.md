...
> Вариант [[ml_concepts/multi-head-attention|multi-head attention]], в котором все $h$ heads делят одну общую пару $K, V$, но каждая имеет свою $Q$. Сокращает [[ml_concepts/kv-cache|KV-кэш]] в $h$ раз ценой потери выразительности: heads вынуждены работать в одном и том же KV-подпространстве.
...
- [[ml_concepts/multi-head-attention]] — исходная схема, относительно которой MQA — крайний случай шаринга.
- [[ml_concepts/grouped-query-attention]] — промежуточная точка: heads делятся на группы, каждая группа имеет свой $K, V$. Снимает значительную часть потери качества MQA.
- [[ml_concepts/multi-latent-attention]] — альтернативный подход к сжатию KV (через латентное представление, а не через шаринг).
- [[ml_concepts/kv-cache]] — bottleneck, ради которого MQA вообще появился.
...
- [[ml_concepts/grouped-query-attention]] — компромисс, который снимает потерю выразительности MQA, не отказываясь от выигрыша по памяти.
- [[ml_concepts/multi-latent-attention]] — альтернативная стратегия: сжимать кэш, не шаря heads.
