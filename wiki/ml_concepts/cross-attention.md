...
В энкодер-декодер трансформере cross-attention применяется multi-head (см. [[ml_concepts/attention/multi-head-attention]]) и стоит как *второй* подслой в каждом блоке декодера: после masked self-attention и до feed-forward.

## Variations and related concepts

- [[ml_concepts/attention/self-attention]] — частный случай, когда оба потока — это один и тот же $H$. Cross-attention обобщает на два разных источника.
- [[ml_concepts/attention/multi-head-attention]] — cross-attention в трансформере всегда multi-head.
- [[ml_concepts/attention/causal-masking]] — на cross-attention не применяется: декодер имеет право смотреть на весь вход целиком.
- [[methods/architectures/transformer]] — cross-attention — это второй подслой каждого блока декодера, мост между двумя стэками.
...
- [[ml_concepts/attention/causal-masking]] — другая модификация attention, нужная для авторегрессионной генерации в декодере.
- [[methods/architectures/transformer]] — как cross-attention встроен в каждый блок декодера между masked self-attention и feed-forward.
