...
```mermaid
flowchart LR
    subgraph VF[Vector field v of x and t]
        X1[x at t] -->|v of x t| S1[Euler step]
        S1 --> X2[x at t plus dt]
        X2 -->|v| S2[Euler step]
        S2 --> X3[...]
        X3 --> XN[x at zero after 50 to 200 calls]
    end
    subgraph FM[Flow map F of x t s]
        Y1[x at t] -->|F of x t s| YN[x at s in one call]
    end
```

*Diagram: vector field требует много шагов солвера от $t$ к $0$; flow map даёт endpoint напрямую за один forward pass.*
