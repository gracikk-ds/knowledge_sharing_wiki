...
Последняя строка — практический подвох: имея flow map, нельзя подключить его к Heun, DPM-Solver и т.п., потому что нет производной для интегрирования. Сэмплирование делается выделенными расписаниями ([[methods/consistency-model-sampling]] stub).
