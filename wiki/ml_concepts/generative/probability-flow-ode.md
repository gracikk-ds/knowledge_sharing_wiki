...
Стаб. Текущая вики использует probability-flow ODE только как кривую, вдоль которой определена [[ml_concepts/generative/consistency-function]]. В типичной параметризации (EDM-стиль):

$$
\mathrm{d}x \;=\; -\,t\,\nabla_x \log p_t(x)\,\mathrm{d}t.
$$

Вместе с приближением условного score $\nabla_x \log p_t(x) \approx -(x - \mathbb{E}[x_0 \mid x])/t^2$ это и есть ODE, который [[methods/distillation/consistency-training]] линеаризует в прямой reference-путь.
