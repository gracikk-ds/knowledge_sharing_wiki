...
4. Когда готов опубликовать — Claude сам прогонит `/wiki-lint` и сделает `git push origin <ветка>`. Vercel автоматически задеплоит при push в `main`.

**Push разрешён** — Claude может пушить без отдельного апрува. Force-push и пуш в `main` из feature-ветки всё ещё требуют твоего явного «ок».
