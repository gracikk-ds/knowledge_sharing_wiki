...
Если ты впервые в репо — запусти `/onboard`. Это интерактивный гид, который проведёт по шагам: посмотрит реальное состояние репо (что в `raw/`, что в `wiki/`, какие скиллы доступны) и проведёт от «открыл репо» до «сделал первый разбор и опубликовал».
